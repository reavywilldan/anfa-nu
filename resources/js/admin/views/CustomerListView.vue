                            ...
                            <div class="card-body px-0 pt-0 pb-2">
                                <div class="table-responsive p-0">
                                    <table class="table align-items-center mb-0">
                                        ...
                                        <tbody>
                                            <tr v-for="customerList in customerLists" :key="customerList.id"
                                                :id="customerList.id">
                                                ...
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-xs font-weight-bold">
                                                        {{ customerList.gender == 'male' ? 'Pria' : 'Wanita' }}
                                                    </span>
                                                </td>
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-xs font-weight-bold">
                                                        {{ customerList.identity_number }}
                                                    </span>
                                                </td>
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-xs font-weight-bold">
...
import customerListServices from '../../services/customerList'
    ...
    methods: {
        async loadDataFromServer() {
            try {
                ...
                const result = await customerListServices.getCustomerList(obj, this.user.bearer)

                if (result.data.length) {
                    this.customerLists.push(...result.data);
                    this.page++;
                } else {
                    this.noResult = true;
        ...
        async onKeyupLoadDataFromServer() {
            try {
                ...
                if (result.data.length) {
                    this.customerLists = result.data
                }
            } catch (err) {
                this.noResult = true;
                this.message = "Error loading data";
